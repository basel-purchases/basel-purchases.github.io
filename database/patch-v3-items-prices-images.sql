-- Basel Purchases V6 database patch
-- Run ONCE after setup-v2.sql / supabase-patch-v2.sql.
-- Adds item-level multiple images and the new pricing fields.

begin;

alter table public.requests
  add column if not exists final_price numeric(18,2);

update public.requests
set final_price = initial_price
where final_price is null and initial_price is not null;

alter table public.purchase_items
  add column if not exists last_entry_price numeric(18,2),
...
-- Add non-negative validation only when it does not already exist.
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'purchase_item_nonnegative_last_entry_price') then
    alter table public.purchase_items
      add constraint purchase_item_nonnegative_last_entry_price
      check (last_entry_price is null or last_entry_price >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'purchase_item_nonnegative_unit_price') then
    alter table public.purchase_items
      add constraint purchase_item_nonnegative_unit_price
      check (unit_price is null or unit_price >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'purchase_item_nonnegative_total_price') then
    alter table public.purchase_items
      add constraint purchase_item_nonnegative_total_price
      check (total_price is null or total_price >= 0);
  end if;

  if not exists (select 1 from pg_constraint where conname = 'purchase_item_single_source_price') then
    alter table public.purchase_items
      add constraint purchase_item_single_source_price
      check (last_entry_price is null or unit_price is null);
  end if;
end $$;

alter table public.attachments
  add column if not exists purchase_item_id uuid references public.purchase_items(id) on delete cascade;

create index if not exists attachments_purchase_item_idx
  on public.attachments (purchase_item_id, created_at);

-- Keep the existing RPC signature so the public web client does not need a new privileged API.
-- p_initial_price is retained as the parameter name for backwards compatibility, but V6 sends
-- the FINAL request price in this parameter and stores it in final_price as well.
