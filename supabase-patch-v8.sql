-- Basel Purchases V9.2 database patch
-- Run ONCE after the existing V9/V9.1 patches.
-- V9.2 keeps request_number as TEXT but removes the old digits-only validation.
-- Examples now accepted: 171-172, 171/172, A-17, or any other non-empty text.
...
begin
  ...
  if p_request_number is null or btrim(p_request_number) = '' then
    raise exception 'Request number is required';
  end if;
